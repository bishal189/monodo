            ...
            <section className="rounded-3xl bg-white/10 border border-white/15 shadow-inner shadow-black/10 px-5 py-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-base font-semibold text-white mt-1">Product List ({productCount})</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {products.map((product) => (
                  <div
                    key={product.id}
                    className="rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 shadow-lg shadow-black/10 overflow-hidden hover:shadow-xl hover:shadow-black/20 transition-all duration-300"
                  >
                    {product.image_url && (
                    ...
                    <div className="p-5 space-y-4">
                      <div>
                        <h3 className="text-white font-bold text-lg mb-2">{product.title ?? "N/A"}</h3>
                        <p className="text-pink-300 text-xl font-bold">{formatCurrency(product.price ?? 0)}</p>
                      </div>

                      {product.review_status === "COMPLETED" ? (
                        <button
                          type="button"
                          disabled
                          className="w-full bg-gradient-to-r from-emerald-500 to-green-500 text-white font-semibold py-3 rounded-xl cursor-not-allowed flex items-center justify-center gap-2 shadow-md shadow-emerald-500/30"
                        >
                        ...
                        <div className="space-y-3">
                          <select
                            value={selectedReviews[product.id] || ""}
                            onChange={(e) => handleReviewChange(product.id, Number(e.target.value))}
                          ...
                          <button
                            ...
                            disabled={!selectedReviews[product.id] || submittingProducts[product.id]}
